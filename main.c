...
int main(void)
{
  ...
  while (1)
  {
    // Запуск одного циклу перетворення
    HAL_ADC_Start(&hadc1);
    ...
    if (HAL_ADC_PollForConversion(&hadc1, 10) == HAL_OK)
    {
      ...
      /* Логіка керування світлодіодом (PC13)
         У Blue Pill: GPIO_PIN_RESET вмикає світлодіод, GPIO_PIN_SET вимикає */
      if (voltage > 1.5f)
      {
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, GPIO_PIN_RESET); // СВІТИТЬ
      }
      else
      {
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, GPIO_PIN_SET);   // НЕ СВІТИТЬ
      }
    }
